...
if (length(geo_fn)==0) { # matrix files are alreadly downloaded
  ...
  if (length(gselms)>1) {  # multiple platform
    ...
    if (geo_GPL=="") {stop("This study has multiple platforms. Please assign the platform to the variable geo_GPL in the session coding.")}
    else {cat("Use platform", geo_GPL, "\n"); idx=which(grepl(geo_GPL,gpls))}
  } else {idx=1}
  gse <- gselms[[idx]]
