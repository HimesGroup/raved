...
```{r geo_download, eval=T, echo=F, warning=F, message=F}
...
```{r pheno_new, eval=T, warning=F, echo=F, results="asis"}
...
```{r cel_utility, eval=T, echo=F}
# The celdownload_func function downloads the .cel files from GEO and unextract the zip file
celdownload_func <- function() {
...
# The celall_func function obtains all .cel files in the data directory with full path
celall_func <- function() {list.files(path=paste0(datadir,"/",geo_id,"/data"),pattern=".CEL.gz|.cel.gz",full.names=T)}
# The sampall_func function obtains all samples whose .cel files exist
sampall_func <- function() {basename(gsub("^(.*).(cel|CEL).gz","\\1",celall_func()))}
# The celfiles_func function obtains .cel files of interest with full path
celfiles_func <- function() {
  cel_all=celall_func()
  samp_all=sampall_func()
  samp_of_interest=as.character(pheno$GEO_ID)
  cel_all[samp_all%in%samp_of_interest]
}
```

Download raw data files (.cel) if available.

```{r cel_download, eval=T, echo=F, message=FALSE}
# Check whether the .cel files already exist. Otherwise download from GEO
samp_exist=sampall_func()
samp_of_interest=as.character(pheno$GEO_ID)
if (length(samp_exist)==0) {
  celdownload_func()
} else {
  if (length(intersect(samp_exist,samp_of_interest))!=length(samp_of_interest)) {celdownload_func()}
}
samp_exist=sampall_func()
if (length(intersect(samp_exist,samp_of_interest))!=length(samp_of_interest)) {stop("The .cel files obtained from GEO do not include all the samples of interest")}
```

Read in the raw data and generate an object "raw.data" under the ExpressionFeatureSet (oligo class).

```{r cel_readin, eval=T, echo=F, message=F, warning=F, results="hide"}
celfiles=celfiles_func()
raw.data <- read.celfiles(celfiles)
```

Obtain scan date information from the raw.data for batch effect adjustment

```{r scandate, eval=T, echo=F, results="asis"}
...
Read in the pre-prepared phenotype file.
```{r pheno_readin, eval=T, echo=F}
pheno_fn=paste0(resdir,"/",geo_id,"_Phenotype_withoutQC.txt")
pheno <- read.table(pheno_fn, header=T, sep="\t")
```

Read in the pre-downloaded raw intensity data.

```{r rawdata_readin, eval=T, echo=F, message=F, warning=F, results="hide"}
celfiles=celfiles_func()
raw.data <- read.celfiles(celfiles)
...
rowname <- gsub("^(.*).(cel|CEL).gz","\\1",row.names(pData(raw.data)))
if (!identical(rowname,as.character(pData(raw.data)$GEO_ID))) {stop("The sample names derived from expression data do not match those in phenotype file. Please check!")}
```
...
The log2-transformed intensity distributions of all samples (arrays) are expected to have the similar scale (i.e. the similar positions and widths of the boxes). Outlier detection is applied by computing a Kolmogorov-Smirnov statistic (Ka) between log-intensity distribution for one array and the pooled array data, where an array with a Ka beyond the upper whisker is designated as an outlier.
...
raw_intensity_func <- function() {
  Mss=log2(subsamp(exprs(raw.data))) # use log2 transformed raw probe intensity
...
1. Outlier detection for log2 raw probe intensity
...
outlier_intensity = names(res_intensity$outlier)
cat(length(outlier_intensity), "outlier(s) are detected in the raw intensity metrics.\n")
if (length(outlier_intensity)>0) {cat("They are: ", outlier_intensity)}
```
...
```{r raw_intensity_densplot, eval=T, echo=F}
...
if (length(outlier_MA)>0) {cat("They are: ", outlier_MA)}
```
...
if (length(outlier_spatial)>0) {cat("They are: ", outlier_spatial)}
```
...
```{r RLE_utility, eval=T, echo=F, warning=F, message=F}
...
if (length(outlier_RLE)>0) {cat("They are: ", outlier_RLE)}
```
...
```{r RLE_plot, eval=T, echo=F, warning=F, message=F, fig.height=8, fig.width=6}
res_RLE$plot
```
...
if (length(outlier_NUSE)>0) {cat("They are: ", outlier_NUSE)}
```
...
Distance between arrays is evaluated using mean absolute difference of log-intensity between each pair of arrays, where the hierarchical tree between arrays is created based on the distance, which is visualized by a heatmap and dendrogram.
...
if (length(outlier_dist)>0) {cat("They are: ", outlier_dist)}
```
...
pca_func <- function(pc) {
  group_vars=c("ScanDate_Group", "Disease", "Treatment", "Subject")
  legends=c("ScanDate_Group", "Disease", "Treatment", "Donor")
...
tbQC_func <- function(outliers) { # define outliers
  ...
  pheno_fn=paste0(resdir,"/",geo_id,"_Phenotype_withQC.txt") # save in a new phenotype file
  write.table(tb,pheno_fn,col.names=T,row.names=F,sep="\t",quote=F)
  # defined outliers
