...
if (!(platform %in%c("Affymetrix","Illumina","Agilent"))) {stop("Only Affymetrix, Illumina, Agilent are available platforms.")}
```
...
**Required column: ** GEO_ID, Tissue, Disease, Treatment, Donor (if cells are treated in vitro). For disease-only study, assign "baseline" to Treatment column. For treatment-only study, assign "healthy" or the disease type used to Disease column.

Column naming is rigid for these columns, because pipeline scripts will recognize these name strings, although the column order can be changed.
