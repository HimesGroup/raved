...
**Note that** three variables, platform (platform), suppldata (whether supplementary data are available), and normdata (whether the expression matrix is normalized), need to be **manually** re-defined in the following steps after look into the datasets. A shortname_func function is suggested to be updated.
...
```{r rawdata_affy, eval=T, echo=F, results="hide"}
...
If gene expression matrix data is used, check if they are normalized/log-transformed. After **manual inspection**, assign a logistic variable "normdata" (whether needs log2 transformation for QC). Note that if the data are normalized, it is not likely to detect the outliers based on the intensity metrices.

```{r check_norm, eval=T, echo=F}
if (!(suppldata)) {
  boxplot(exprs(gse),col=colour_list,main="Probe Intensity matrix",xaxt="n")
...
```{r pheno_withoutQC, eval=F, warning=F, echo=F}
...
Use the pre-prepared phenotype file.
...
raw_intensity_func <- function() {
  ...
  else {Mss=subsamp(exprs(raw.data))}
...
if (platform=="Affymetrix") {RNAdeg_func()}
```
...
if (platform=="Affymetrix") {PMMM_func()}
```
...
MA_func <- function(){
  # Compute M-A metrics
  if (!normdata) {sMA <- MAcal_func(log2(exprs(raw.data)))}
  else {sMA <- MAcal_func(exprs(raw.data))}
...
if (platform=="Affymetrix") {res_RLE$plot}
```
...
if (platform=="Affymetrix") {res_NUSE$boxplot}
```
...
dist_func <- function() {
  if (!normdata) {m <- dist2(log2(exprs(raw.data)))}
  else {m <- dist2(exprs(raw.data))}
...
pcastat_func <- function() {
  ...
  if (!normdata) {raw.data.pca <- na.omit(log2(exprs(raw.data)))}
  else {raw.data.pca <- na.omit(exprs(raw.data))}
