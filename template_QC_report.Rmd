...
if (nlevels(pheno.raw$supplementary_file)==1&levels(pheno.raw$supplementary_file)=="NONE") {suppldata=FALSE} else {suppldata=TRUE}
```
...
vars=c("ScanDate_Group","Disease","Treatment")
...
if (!suppldata) {
  boxplot(exprs(raw.data),col=colour_list,main="Probe Intensity matrix of raw data",xaxt="n")
  legend("topright",legend=names(colour_list),fill=colour_list,cex=0.8)
  if (!normdata) {
    boxplot(log2(exprs(raw.data)+1),col=colour_list,main="Probe Intensity matrix of log2 data",xaxt="n")
    legend("topright",legend=names(colour_list),fill=colour_list,cex=0.8)
...
if (platform=="Affymetrix"&suppldata) {RNAdeg_func()}
```
...
MA_func <- function(){
  # Compute M-A metrics
  if (!normdata) {
    # check if any zero intensity value in the expression data
    if (any(apply(exprs(raw.data),2,min)<=0)) {sMA <- MAcal_func(log2(exprs(raw.data)+1))} # add constant 1 to raw data intensity
    else {sMA <- MAcal_func(log2(exprs(raw.data)))}
  }
...
dist_func <- function() {
  if (!normdata) {
    # check if any zero intensity value in the expression data
    if (any(apply(exprs(raw.data),2,min)<=0)) {m <- dist2(log2(exprs(raw.data)+1))} # add constant 1 to raw data intensity
    else {m <- dist2(log2(exprs(raw.data)))}
  }
...
pcastat_func <- function() {
  ...
  if (!normdata) {
    if (!normdata) {raw.data.pca <- na.omit(apply(log2(exprs(raw.data)),2,function(x)replace(x,is.infinite(x),NA)))}
  }
  else {raw.data.pca <- na.omit(apply(exprs(raw.data),2,function(x)replace(x,is.infinite(x),NA)))} # replace infinite values to NAs and omit NAs
