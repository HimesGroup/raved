...
if (length(treatment)==2) {paired=TRUE} else if (length(disease)==2) {paired=FALSE}
```

```{r pheno_paired, eval=T, echo=F, results="asis"}
if (paired) {
...
if (usesuppl) {
  ...
  matching <- mapply(grepl, x=samp_exist, pattern=samp_need)
  if (FALSE %in% matching) {stop("The .cel files obtained from GEO do not include all the samples of interest")}
...
glpanno_func <- function(GLP_ID){
  gpl <- Table(getGEO(GLP_ID, destdir = "data"))
  anno_vect <- gpl[,"GENE_SYMBOL"]
...
geneannot_func <- function(tb, anno_vect) {
  ...
  tb$SYMBOL=sapply(as.character(tb$ID), function(x){anno_vect[x]})
  tb[order(tb$P.Value),]
}
```
...
if (!anno_array%in%names(anno_list)) {
  ...
  anno_vect=glpanno_func(GLP_ID=anno_array)
} else {
