...
phenosub_func <- function(){
  ...
  if (identical(sort(levels(pheno.sub$Treatment)),sort(c(con0,con1)))) {pheno.sub$Status=pheno.sub$Treatment} else {pheno.sub$Stuatus=pheno.sub$Disease}
...
# The celreadin_func function reads in raw data as an oligo object
celreadin_func <- function(celfiles, pheno) {
...
Download raw data files (.cel) or GEO dataset if they do not exist.

```{r cel_download, eval=T, echo=F, message=FALSE}
# Check whether the .cel files already exist. Otherwise download from GEO
celfiles=paste0(datadir,"/",geo_id,"/data/",pheno.of.interest$Filename)
celfile_exist=celall_func()
if (length(celfile_exist)==0) {
  celdownload_func()
} else {
  if (length(intersect(celfile_exist,celfiles))!=length(celfiles)) {celdownload_func()}
}
celfile_exist=celall_func()
if (length(intersect(celfile_exist,celfiles))!=length(celfiles)) {stop("The .cel files obtained from GEO do not include all the samples of interest")}
```
...
raw.data.of.interest=celreadin_func(celfiles=celfiles, pheno=pheno.of.interest)
```
...
```{r rma, eval=T, echo=F, message=FALSE, results="hide"}
...
contrast_table <- data.frame(ID=row.names(contrast_table), contrast_table)
```
...
Assign official gene symbol using hugene20sttranscriptcluster.db if annotation is based on pd.hugene.2.0.st.v1.

```{r anno_utility, eval=T, echo=F}
geneannot_func <- function(tb) {
  remove.pm <- function(x) {gsub("_PM", "", x)}
  tb$ID=sapply(as.character(tb$ID), remove.pm)
  anno_array=annotation(rma.data.of.interest)
  cat("The platform used is ", anno_array, "\n")
  if (grepl("u133",anno_array)) {
    anno_lib="hgu133plus2SYMBOL"
  } else if (grepl("u133",anno_array)) {
    anno_lib="hugene10sttranscriptclusterSYMBOL"
  }
  tb$SYMBOL=sapply(as.character(tb$ID), function(x) mget(x, env=get(anno_lib), ifnotfound=NA)[[1]])
  tb[order(tb$P.Value),]
}
```

```{r anno_tb, eval=F, echo=F, warning=F,message=F, results="asis"}
...
```{r vol, echo=F, eval=T, fig.height=4, fig.width=4.5}
...
topgene_boxplot_func <- function(tb,colour,comp) { # comp: comparison status
  for (i in 1:nrow(tb)) {# top i probe in res
    ...
    title=paste0(comp," top ",i," probe ", probe_top)
    print(
