...
if (length(disease)==2) {
  ...
  if (all(table(pheno.sub$Donor)==1)) {paired=FALSE} # if treatment is in different donors
...
GPL_ID=sapply(getGEO(geo_id),annotation)
```


```{r primeview_check, eval=T, echo=F}
if (geo_GPL=="") {if (length(GPL_ID)>2) {stop("More than one platforms exist. Define geo_GPL variable.")}} else {GPL_ID=geo_GPL}
if (GPL_ID=="GPL15207") {
  ...
  if (usesuppl) {stop("set usesuppl==TRUE ", "and inspect whether gse gene expression matrix is normalized.")}
}
```
...
if (paired) {
  ...
  if (batcherror=="no") cat("Both scan date and donor are adjusted.\n")
    pandoc.table(res_svamod$tb.fullmod, split.tables=Inf, caption="Full model matrix")
    pandoc.table(res_svamod$tb.nullmod, split.tables=Inf, caption="Null model matrix")
}
```
...
symbol_list=list()
# Agilent
symbol_list[["GPL17077"]]="GENE_SYMBOL"
symbol_list[["GPL15207"]]="Gene Symbol"
symbol_list[["GPL4133"]]="GENE_SYMBOL"
```
