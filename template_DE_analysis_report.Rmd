...
batcherror_func <- function() {
  if (nbatch==0) {
    ...
    if (batchadj) { if (donoradj) {batcherror="no"} else {batcherror="correlate_donor_scandate"}} else {batcherror="correlate"}
  }
  return(batcherror)
}
...
batchadj_func <- function(batcherror=batcherror, contrast_table=contrast_table, paired=paired) {
  ...
  } else if (batcherror=="correlate"|batcherror=="correlate_donor_scandate") {
