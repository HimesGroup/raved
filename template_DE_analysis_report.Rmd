...
If any intensity value is zero in expression matrix, add constant one to the intensity matrix, as hierarchical clustering cannot handle infinite/NA value.

```{r data_nozero, eval=T, echo=F, fig.height=10, fig.width=12}
# check if any zero intensity value in the expression data
if (any(apply(exprs(raw.data.of.interest),2,min)<=0)) {
  if (!(usesuppl)) {
    if (!normdata) {rma.exprs.nozero=normalize.quantiles(log2(exprs(raw.data.of.interest)+1))}
    else {rma.exprs.nozero=exprs(rma.data.of.interest)}
  } else {
    raw.data.of.interest2=raw.data.of.interest
    exprs(raw.data.of.interest2)=exprs(raw.data.of.interest2)+1
    rma.exprs.nozero=exprs(rma(raw.data.of.interest2))
  }
} else {
    rma.exprs.nozero=exprs(rma.data.of.interest)
}
```

```{r corplot1, eval=T, echo=F, fig.height=10, fig.width=12}
corplot_func(m=rma.exprs.nozero,colour_status_list=colour_status_list, colour_status=colour_status, main="Correlation heatmap by comparison status")

```

```{r corplot2, eval=T, echo=F, fig.height=10, fig.width=12}
if ("ScanDate_Group"%in%names(pheno)) {
  corplot_func(m=rma.exprs.nozero,colour_status_list=colour_scandate_list, colour_status=colour_scandate, main="Correlation heatmap by scan date")
}
```
