---
title: 'Quality Control for Gene Expression Microarray Data -- GSE4302'
...
```{r, eval=F, warning=F}
cols <- c("title","geo_accession","source_name_ch1","characteristics_ch1")
cols <- c("title","geo_accession","source_name_ch1","characteristics_ch1")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(Sample=paste(geo_accession,title,sep="_")) %>%
  dplyr::mutate(Subject=as.numeric(sub("\\D*(\\d+).*", "\\1",title))) %>%
  dplyr::mutate(Tissue="BE") %>%
  dplyr::mutate(Disease=ifelse(grepl("Asthmatic",characteristics_ch1),"asthma",ifelse(grepl("Healthy",characteristics_ch1),"healthy","smoker"))) %>%
  dplyr::mutate(Treatment=ifelse(grepl("Flovent",characteristics_ch1),"fluticasone",ifelse(grepl("Placebo",characteristics_ch1),"placebo","baseline"))) %>%
  dplyr::rename(GEO_ID=geo_accession) %>%
  dplyr::select(Sample,Treatment,Tissue,Disease,Subject,GEO_ID) %>%
...
```{r eval=T}
disease_con1 <- c("asthma","smoker")
disease_con0 <- c("healthy","healthy")
treatment_con1 <- c("fluticasone",NULL)
treatment_con0 <- c("placebo","baseline")
```
...
pheno <- read.table(paste0(datadir,"/",geo_id,"_Phenotype_withoutQC.txt"),header=T)
...
```{r, eval=T}
pheno$ScanDate_Group <- sapply(strsplit(as.character(protocolData(raw.data)$dates), " "), function(x) {x[[1]]} )
```
...
```{r, eval=T, warning=F, message=F,fig.height=12, fig.width=7}
# boxplot function
...
PM <- log2(pm(raw.data))
MM <- log2(mm(raw.data))
if(nrow(PM) != nrow(MM)) {message("This might be a PM-only array. No plots will be generated.")} else {
  subsample=20000
  ...
  cols <- colours[1:length(levels(as.factor(types)))]
  ggplot(data.frame(values,types),aes(x=values,colour=types)) + geom_line(aes(group=types),stat="density") +
    theme_bw() +
    xlab("Intensity") +
    ylab("Density") +
...
status.cols <- unlist(lapply(pData(raw.data)$ScanDate_Group,function(x)colour_list[x])) # colour list to corresponding scan date list
heatmap.2(m,col=viridis(256, option="B"),ColSideColors=status.cols,RowSideColors=status.cols,
...
1. PCA plot
```{r, eval=T, warning=F, message=F}
pca <- prcomp(t(na.omit(raw.data.log2)),retx = TRUE, center = TRUE, scale = TRUE)
pc <- data.frame(pca$x)
cols <- colour_list
df <- data.frame(
  PC1=pc$PC1,
...
2. Variance explained by the first 10 PCs
...
res <- data.frame(rownames(pcs),pvars,cumsum_pvars)[1:10,]
...
pData(raw.data)$QC_Pass <- rep(NA,nrow(pheno)) # add column QC_Pass in phnotype data
