---
title: 'Quality Control for Gene Expression Microarray Data -- GSE35571'
...
```{r eval=F, warning=F}
cols <- c("title","geo_accession","characteristics_ch1","characteristics_ch1.1","characteristics_ch1.2")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(GEO_ID=geo_accession) %>%
  dplyr::mutate(Subject=gsub("child-","",title)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="PBMC") %>%
  dplyr::mutate(Disease=ifelse(grepl("disease status: asthmatic",characteristics_ch1.2),"asthma",ifelse(grepl("disease status: non-asthmatic",characteristics_ch1.2),"healthy",NA))) %>%
  dplyr::mutate(Age=gsub("age \\(yr\\): ","",characteristics_ch1)) %>%
  dplyr::mutate(Gender=ifelse(grepl("gender: F",characteristics_ch1.1),"female","male")) %>%
...
pc <- data.frame(pca$x)
pc$sdev <- pca$sdev
write.table(pc,paste0(resdir,"/",geo_id,"_pca.txt"),col.names=T,row.names=F,sep="\t",quote=F)
```

Note: in this study, due to the large sample size, we saved the pre-computed pcs and sdev into in a table to avoid computing pca in the knit step.

compute variance explained by each PC

```{r eval=T, warning=F, message=F, results="asis"}
pc <- read.table(paste0(resdir,"/",geo_id,"_pca.txt"),header=T,sep="\t")
vars <- pc$sdev^2
pcs <- t(pc[,grepl("PC",names(pc))]) # get rid of column sdev
