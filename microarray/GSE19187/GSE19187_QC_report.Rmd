---
title: 'Quality Control for Gene Expression Microarray Data -- GSE19187'
...
```{r eval=T, warning=F}
cols <- c("title","geo_accession","characteristics_ch1","characteristics_ch1.2","characteristics_ch1.3")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(GEO_ID=geo_accession) %>%
  dplyr::mutate(Subject=gsub("^.* (.*)$","\\1",title)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="NE") %>%
  dplyr::mutate(Disease=ifelse(grepl("disease state: Rhinitis with controlled asthma",characteristics_ch1),"rhinitis_controlled_asthma",ifelse(grepl("disease state: Uncontrolled asthma",characteristics_ch1),"uncontrolled_asthma",ifelse(grepl("Healthy",characteristics_ch1),"healthy","rhinitis")))) %>%
  dplyr::mutate(Age=sub("^age: (.*?) y$","\\1",pheno.df$characteristics_ch1.3)) %>%
  dplyr::mutate(Gender=ifelse(grepl("Female",characteristics_ch1.2),"female","male")) %>%
...
```{r eval=T}
disease_con1 <- c("rhinitis_controlled_asthma","uncontrolled_asthma","rhinitis")
disease_con0 <- c("healthy","healthy","healthy")
treatment_con1 <- NULL
treatment_con0 <- NULL
```
