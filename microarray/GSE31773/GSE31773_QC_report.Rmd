---
title: 'Quality Control for Gene Expression Microarray Data -- GSE31773'
...
```{r eval=T, warning=F}
cols <- c("title","geo_accession","source_name_ch1","characteristics_ch1","characteristics_ch1.2","characteristics_ch1.3")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(GEO_ID=geo_accession) %>%
  dplyr::mutate(Subject=gsub("patient id: ","",characteristics_ch1.3)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue=ifelse(grepl("CD4",source_name_ch1),"CD4","CD8")) %>%
  dplyr::mutate(Disease=ifelse(grepl("Tcells_non_severe_asthma",source_name_ch1),"non_severe_asthma",ifelse(grepl("Tcells_severe_asthma",source_name_ch1),"severe_asthma","healthy"))) %>%
  dplyr::mutate(Age=gsub("age \\(years\\): ","",characteristics_ch1.2)) %>%
  dplyr::mutate(Gender=gsub("gender: ","",characteristics_ch1)) %>%
...
```{r eval=T}
disease_con1 <- c("non_severe_asthma","severe_asthma")
