---
title: 'Quality Control for Gene Expression Microarray Data -- GSE44037'
...
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(Subject=sub("(^[^_]+_[^_]+)_(.*)$", "\\1",title)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue=ifelse(grepl("bronchial",source_name_ch1),"BE","NE")) %>%
  dplyr::mutate(Disease=ifelse(grepl("asthma",source_name_ch1),"asthma",ifelse(grepl("healthy",source_name_ch1),"healthy","rhinitis"))) %>%
  dplyr::mutate(Gender=ifelse(grepl("female",characteristics_ch1),"female","male")) %>%
  dplyr::rename(GEO_ID=geo_accession) %>%
  dplyr::select(Sample,Tissue,Disease,Gender,Subject,GEO_ID) %>%
...
```{r eval=T}
disease_con1 <- c("asthma","rhinitis")
