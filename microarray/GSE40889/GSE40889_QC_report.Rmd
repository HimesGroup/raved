---
title: 'Quality Control for Gene Expression Microarray Data -- GSE40889'
...
geo_fn
if (!file.exists(paste0(datadir,"/",geo_fn))) {
  gselms <- getGEO(geo_id, destdir=datadir,GSEMatrix = TRUE)
  gse <- gselms[[names(gselms)%in%geo_fn]]
} else {
...
```{r eval=F, warning=F}
cols <- c("title","geo_accession","source_name_ch1","characteristics_ch1.2","characteristics_ch1.3")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(GEO_ID=geo_accession) %>%
  dplyr::mutate(Subject=gsub("(^.*\\d+)_.*$","\\1",gsub("(^.*)\ (.*).*$","\\2",title))) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue=ifelse(grepl("CD4",source_name_ch1),"CD4","PBMC")) %>%
  dplyr::mutate(Treatment=ifelse(grepl("anti-CD3",characteristics_ch1.2),"CD328",ifelse(grepl("LpA",characteristics_ch1.2),"LpA","baseline"))) %>%
  dplyr::mutate(Treatment_des=ifelse(grepl("anti-CD3",characteristics_ch1.2),"anti-CD3/CD28 3/1 Aug/ml 48h",ifelse(grepl("LpA",characteristics_ch1.2),"LpA 0.1Aug/ml 48h",NA))) %>%
  dplyr::mutate(Disease=ifelse(grepl("group: allergic asthmatic",characteristics_ch1.3),"allergic_asthma",ifelse(grepl("group: non-allergic asthmatic",characteristics_ch1.3),"non_allergic_asthma",ifelse(grepl("GK",characteristics_ch1.3),"healthy",NA)))) %>%
  dplyr::mutate(Disease_des=ifelse(grepl("group: allergic asthmatic",characteristics_ch1.3),"allergic_asthma",ifelse(grepl("group: non-allergic asthmatic",characteristics_ch1.3),"non_allergic_asthma",NA))) %>%
  dplyr::mutate_if(is.character,as.factor)
pheno.new <- pheno.new[,(length(cols)+1):ncol(pheno.new)]
# assign diesease status to CD4 samples based on the disease status of PBMC from the same donors
samps <- pheno.new$Subject[pheno.new$Tissue=="CD4"][pheno.new$Subject[pheno.new$Tissue=="CD4"]%in%pheno.new$Subject[pheno.new$Tissue=="PBMC"]]
for (samp in samps) {
  pheno.new$Disease_des[pheno.new$Subject==samp&pheno.new$Tissue=="CD4"] <- unique(pheno.new$Disease_des[pheno.new$Subject==samp&pheno.new$Tissue=="PBMC"])
  pheno.new$Disease[pheno.new$Subject==samp&pheno.new$Tissue=="CD4"] <- unique(pheno.new$Disease[pheno.new$Subject==samp&pheno.new$Tissue=="PBMC"])
}
```
...
raw.data.affy <- read.affybatch(list.celfiles(paste0(datadir,"/",geo_id,"/data/"), full.names=TRUE),compress=T)
```
...
Note that as the samples are strongly correlated with the year of scan time, create a column Batch_year for batch effect adjustment in DE analysis. If the compared samples were scanned in the same year, adjust batch effect using scan date; otherwise, adjust using the year for scanning.
```{r eval=T}
pData(raw.data)$Batch_year <- gsub("^(\\d+)-.*$","\\1",pData(raw.data)$ScanDate_Group)
```
...
```{r eval=T}
disease_con1 <- c("allergic_asthma","non_allergic_asthma")
disease_con0 <- c("healthy","healthy")
treatment_con1 <- c("CD328","LpA")
treatment_con0 <- c("baseline","baseline")
```
