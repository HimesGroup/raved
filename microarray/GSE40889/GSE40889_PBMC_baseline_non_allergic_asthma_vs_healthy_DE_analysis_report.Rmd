---
title: 'Microarray Data Analysis -- GSE40889 non_allergic_asthma vs healthy'
...
geo_id="GSE40889"
# phenotype file
pheno_fn="C:/Users/mengykan/Projects/AsthmaApp/res/microarray/GSE40889_PBMC_baseline_non_allergic_asthma_vs_healthy.txt"
...
pandoc.table(res,split.table=Inf)
res <- as.data.frame(table(pData(raw.data.of.interest)[,c("Status","Batch_year")]))
names(res) <- c("Status","Batch_year","Frequency")
pandoc.table(res,split.table=Inf)
```
...
raw.data.of.interest$Batch_year <- as.factor(raw.data.of.interest$Batch_year)
i=nlevels(raw.data.of.interest$Batch_year)
colour_scandate <- colours[1:i]
names(colour_scandate) <- levels(raw.data.of.interest$Batch_year) # colour to corresponding scan date
colour_scandate_list <- unlist(lapply(raw.data.of.interest$Batch_year,function(x){colour_scandate[x]}))
```
...
if (comptype=="Disease") {Description=levels(as.factor(phenoData(raw.data.of.interest)$Disease_des))}
if (comptype=="Treatment") {Description=levels(as.factor(phenoData(raw.data.of.interest)$Treatment_des))}
df <- data.frame(
  GEO_ID=geo_id,
...
NOTE: In this study, because samples are strongly correlated with the year of scan time, use variable Batch_year for batch effect adjustment instead of Batch_year.

Check if there are multiple batches in the study.
```{r eval=T, results="asis"}
nbatch=nlevels(rma.data.of.interest$Batch_year)
```
...
modBatch =""
if (nbatch>1) {
 modBatch = model.matrix(~factor(rma.data.of.interest$Status)+factor(rma.data.of.interest$Batch_year) ) # full model (adjustment variables and variables of interest)
 res <- as.data.frame(modBatch)
 names(res) <- c("Intercept",levels(rma.data.of.interest$Status)[-1],levels(rma.data.of.interest$Batch_year)[-1])
 pandoc.table(res,split.table=Inf)
}
```
...
nullBatch=""
if (nbatch>1) {
  nullBatch =  model.matrix(~factor(rma.data.of.interest$Batch_year)) # null model (adjustment variables only)
  res <- as.data.frame(nullBatch)
  names(res) <- c("Intercept",levels(rma.data.of.interest$Batch_year)[-1])
  pandoc.table(res,split.table=Inf)
}
```
...
df <- data.frame(
  PC1=pc$PC1,
  PC2=pc$PC2,
  group=rma.data.of.interest$Batch_year
)
