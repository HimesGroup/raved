---
title: 'Quality Control for Gene Expression Microarray Data -- GSE61880'
...
pheno.new <- pheno.df %>%
  ...
  dplyr::mutate(Subject=gsub("^.*-(rep\\d+)$","\\1",title)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="MACRO") %>%
  dplyr::mutate(Treatment=ifelse(grepl("dex",title),paste0("dex_",gsub("MDM-(.*h)-dex.*$","\\1",title)),"baseline")) %>%
  dplyr::mutate(Treatment_des=ifelse(grepl("dex",title),paste0("Dex ",gsub("MDM-(.*h)-dex.*$","\\1",title)," 100nM"),NA)) %>%
...
disease_con1 <- NULL
disease_con0 <- NULL
treatment_con1 <- c("dex_10h","dex_1h","dex_24h","dex_2h","dex_4h")
treatment_con0 <- rep("baseline",length(treatment_con1))
```
