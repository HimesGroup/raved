---
title: 'Quality Control for Gene Expression Microarray Data -- GSE51392'
...
**NOTE**: Samples for disease comparison in this study are the same as in GSE44037. Only use this study for treatment comparison.
...
```{r eval=F, warning=F}
cols <- c("geo_accession","characteristics_ch1","characteristics_ch1.1","characteristics_ch1.2","characteristics_ch1.3","characteristics_ch1.4")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(GEO_ID=geo_accession) %>%
  dplyr::mutate(Subject=gsub("\\D", "", characteristics_ch1.2)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue=ifelse(grepl("bronchial_epithelium",characteristics_ch1),"BE","NE")) %>%
  dplyr::mutate(Gender=ifelse(grepl("Female",characteristics_ch1.3),"female","male")) %>%
  dplyr::mutate(Disease=gsub("disease state \\(condition\\): ","",characteristics_ch1.1)) %>%
  dplyr::mutate(Disease_des=ifelse(grepl("asthma",characteristics_ch1.1),"allergic_asthma",NA)) %>%
  dplyr::mutate(Treatment=ifelse(grepl("Poly",characteristics_ch1.4),"ploy_I_C","baseline")) %>%
  dplyr::mutate(Treatment_des=ifelse(grepl("Poly",characteristics_ch1.4),"Poly(I:C) 24Aug/ml 24h",NA)) %>%
