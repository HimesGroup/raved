---
title: 'Quality Control for Gene Expression Microarray Data -- GSE55876'
...
pheno.new <- pheno.df %>%
  ...
  dplyr::mutate(Subject=gsub("^.* (exp.*)$","\\1",title)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="chALL") %>%
  dplyr::mutate(Treatment=ifelse(grepl("DEX",title),"dex",ifelse(grepl("EtOH",title),"baseline",ifelse(grepl("4.0e-6M",title),"solu-dacortin_4uM","solu-dacortin_8uM")))) %>%
  dplyr::mutate(Treatment_des=ifelse(grepl("DEX",title),"Dex 100nM 6h",ifelse(grepl("4.0e-6M",title),"Solu-dacortin4uM 4uM 6h",ifelse(grepl("8.0e-6M",title),"Solu-dacortin 8uM 6h",NA)))) %>%
...
disease_con1 <- NULL
disease_con0 <- NULL
treatment_con1 <- c("dex","solu-dacortin_4uM","solu-dacortin_8uM")
treatment_con0 <- c("baseline","baseline","baseline")
```
