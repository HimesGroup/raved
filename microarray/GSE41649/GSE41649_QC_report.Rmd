---
title: 'Quality Control for Gene Expression Microarray Data -- GSE41649'
...
```{r eval=F, warning=F}
cols <- c("title","geo_accession","source_name_ch1","description")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(GEO_ID=geo_accession) %>%
  dplyr::mutate(Subject=gsub("SAMPLE ","",description)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="BE") %>%
  dplyr::mutate(Disease=ifelse(grepl("allergic_asthmatic",source_name_ch1),"asthma","healthy")) %>%
  dplyr::mutate(Disease_des="allergic_asthma") %>%
