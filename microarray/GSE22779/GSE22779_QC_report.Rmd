---
title: 'Quality Control for Gene Expression Microarray Data -- GSE22779'
...
```{r, eval=F, warning=F}
cols <- c("title","geo_accession","characteristics_ch1","characteristics_ch1.1","characteristics_ch1.2","characteristics_ch1.3","characteristics_ch1.4","characteristics_ch1.5")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(Subject=ifelse(grepl("patient: ",characteristics_ch1.5),gsub("patient: ","",characteristics_ch1.5),"HD")) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="PBMC") %>%
  dplyr::mutate(treatment_time=gsub("\\D", "", characteristics_ch1.4)) %>%
  dplyr::mutate(treatment_drug=ifelse(grepl("dexamethasone",characteristics_ch1.3),"dexamethasone","baseline")) %>%
  dplyr::mutate(Treatment=paste0(treatment_drug,"_",treatment_time,"h")) %>%
  dplyr::mutate(Disease=ifelse(grepl("epilepsy",characteristics_ch1.2),"epilepsy","healthy")) %>%
  dplyr::rename(GEO_ID=geo_accession) %>%
  dplyr::select(Sample,Tissue,Disease,Treatment,Subject,GEO_ID) %>%
...
```{r, eval=T, warning=F, message=F,fig.height=7, fig.width=7}
# boxplot function
...
if(("Treatment"%in%colnames(pData(raw.data)))&(min(summary(pData(raw.data)$Treatment))>2)) {
  ...
  i=length(levels(pheno$Treatment))
...
if(("Treatment"%in%colnames(pData(raw.data)))&(min(summary(pData(raw.data)$Subject))>2)) {
  ...
  i=length(levels(pData(raw.data)$Subject))
...
```{r eval=T}
disease_con1 <- c("epilepsy")
disease_con0 <- NA
treatment_con1 <- c("dexamethasone_2h","dexamethasone_6h","dexamethasone_24h")
treatment_con0 <- c("baseline_0h","baseline_0h","baseline_0h")
```
