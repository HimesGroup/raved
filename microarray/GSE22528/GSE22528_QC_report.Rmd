---
title: 'Quality Control for Gene Expression Microarray Data -- GSE22528'
...
```{r eval=T, warning=F}
cols <- c("title","geo_accession","characteristics_ch1.1")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(GEO_ID=geo_accession) %>%
  dplyr::mutate(Subject=ifelse(grepl("Allergic asthmatic",title),gsub("Allergic asthmatic","asthma",title),gsub("Control","control",title))) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="BAL") %>%
  dplyr::mutate(Disease=ifelse(grepl("asthma",characteristics_ch1.1),"allergic_asthma","healthy")) %>%
