---
title: 'Quality Control for Gene Expression Microarray Data -- GSE35643'
...
```{r eval=T, warning=F}
cols <- c("geo_accession","characteristics_ch1.1","characteristics_ch1.3","characteristics_ch1.4","characteristics_ch1.5","characteristics_ch1.6","characteristics_ch1.7","characteristics_ch1.11","characteristics_ch1.12")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(GEO_ID=geo_accession) %>%
  dplyr::mutate(Subject=gsub("\\D", "", characteristics_ch1.3)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="ASM") %>%
  dplyr::mutate(Gender=gsub("gender: ", "", characteristics_ch1.4)) %>%
  dplyr::mutate(Age=gsub("\\D", "", characteristics_ch1.5)) %>%
  dplyr::mutate(Smoking=gsub("smoker status: ","",characteristics_ch1.6)) %>%
  dplyr::mutate(Disease=ifelse(grepl("asthmatic",characteristics_ch1.7),"asthma","healthy")) %>%
  dplyr::mutate(Disease_des=ifelse(grepl("asthmatic",characteristics_ch1.7),"mild_asthma",NA)) %>%
  dplyr::mutate(Treatment=ifelse(grepl("IL-17",characteristics_ch1.1),"IL17","baseline")) %>%
  dplyr::mutate(Treatment_des=ifelse(grepl("IL-17",characteristics_ch1.1),"IL-17 10ng/ml 2h",NA)) %>%
  dplyr::mutate(Therapy=gsub("therapy: ", "", characteristics_ch1.11)) %>%
  dplyr::mutate(Atopy=gsub("atopy: ", "", characteristics_ch1.12)) %>%
...
pheno <- read.table(paste0(datadir,"/",geo_id,"_Phenotype_withoutQC.txt"),sep="\t",header=T)
