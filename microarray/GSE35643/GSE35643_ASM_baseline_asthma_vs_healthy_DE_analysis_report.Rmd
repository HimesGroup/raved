---
title: 'Microarray Data Analysis -- GSE35643 asthma vs healthy'
...
geo_id="GSE35643"
# phenotype file
pheno_fn="C:/Users/mengykan/Projects/AsthmaApp/res/microarray/GSE35643_ASM_baseline_asthma_vs_healthy.txt"
...
```{r eval=T, message=FALSE, results="asis"}
if (annotation(raw.data)=="pd.hg.u133.plus.2") {
  library(hgu133plus2.db)
  remove.pm <- function(x) {gsub("_PM", "", x)}
  contrast_table_adjust$ID = sapply(contrast_table_adjust[,1], remove.pm)
  contrast_table_adjust$SYMBOL <- lapply(as.character(contrast_table_adjust$ID), function(x) mget(x, env=hgu133plus2SYMBOL, ifnotfound=NA)[[1]])} else if (annotation(raw.data)=="pd.hugene.1.0.st.v1") {
...
```{r eval=T}
if (nbatch==1) {"There is only one batch in this study."} else {
