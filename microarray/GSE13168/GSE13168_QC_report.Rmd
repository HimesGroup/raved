---
title: 'Quality Control for Gene Expression Microarray Data -- GSE13168'
...
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(Subject=sub("(^[^_]+_[^_]+)_(.*)$", "\\2",title)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="ASM") %>%
  dplyr::mutate(Treatment=gsub("\\+","_",sub("(^[^_]+_[^_]+)_(.*)$", "\\1",title))) %>%
  dplyr::rename(GEO_ID=geo_accession) %>%
  dplyr::select(Sample,Tissue,Treatment,Subject,GEO_ID) %>%
...
Appearant clusters of scan date are observed. Check if the clustering is associated with the same treatment condition.
```{r, eval=T, warning=F, message=F}
# Assign colour to treatment status
i=length(levels(pheno$Treatment))
colour_treat <- colours[1:i]
names(colour_treat) <- levels(pheno$Treatment) 
cols <- colour_treat
df <- data.frame(
  PC1=pc$PC1,
  PC2=pc$PC2,
  scandate=pData(raw.data)$Treatment
)
ggplot(df,aes(PC1,PC2,color=scandate)) + geom_point() +
  theme_bw() +
...
As it is unlikely to link clustering with the treatment conditions, check if the clusters can be assigned to the same donor under different treatment conditions. It seems that cells from the same donor were measured in the same day. Overall, the clustering is likely due to scanning in different times.
```{r, eval=T, warning=F, message=F}
# Assign colour to subject status
i=length(levels(pheno$Subject))
...
df <- data.frame(
  PC1=pc$PC1,
  PC2=pc$PC2,
  scandate=pData(raw.data)$Subject
)
ggplot(df,aes(PC1,PC2,color=scandate)) + geom_point() +
  theme_bw() +
...
disease_con1 <- NULL
disease_con0 <- NULL
treatment_con1 <- c("GFP_Flu","GFP_Flu_EGF","GFP_Flu_IL1b","GFP_Flu_E_I","PKI_basal")
treatment_con0 <- c("GFP_basal","GFP_EGF","GFP_IL1b","GFP_E_I","GFP_basal")
```
