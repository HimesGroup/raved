---
title: 'Microarray Data Analysis -- GSE22152 dex_sens vs baseline_sens'
...
geo_id="GSE22152"
# phenotype file
pheno_fn="C:/Users/mengykan/Projects/AsthmaApp/res/microarray/GSE22152_chALL_dex_sens_vs_baseline_sens.txt"
...
celfiles <- paste0(datadir,"/",geo_id,"/data/",pheno$Filename) # get .cel file names to read in. This step is important for samples scanned in different platforms
...
if (nbatch>1) {
 ...
 names(res) <- c("Intercept",levels(rma.data.of.interest$Status)[-1],levels(droplevels(rma.data.of.interest$ScanDate_Group))[-1])
 pandoc.table(res,split.table=Inf)
}
```
...
if (nbatch>1) {
  ...
  names(res) <- c("Intercept",levels(droplevels(rma.data.of.interest$ScanDate_Group))[-1])
  pandoc.table(res,split.table=Inf)
}
```
...
Assign official gene symbol using hgu95av2.db if annotation is based on pd.hg.u95a and pd.hg.u95av2.
...
} else if (grepl("u95a",annotation(raw.data))) {
  library(hgu95av2.db)
  contrast_table_adjust$ID = contrast_table_adjust[,1]
  contrast_table_adjust$SYMBOL <- lapply(as.character(contrast_table_adjust$ID), function(x) mget(x, env =hgu95av2SYMBOL, ifnotfound=NA)[[1]]) 
}
...
heatmap_topgene <- function(dt,status.cols) {
  array_name <- gsub(".CEL.gz|.cel.gz","",colnames(dt)) # shorten the sample id
