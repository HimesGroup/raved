---
title: 'Quality Control for Gene Expression Microarray Data -- GSE22152'
...
pheno.new <- pheno.df %>%
  ...
  dplyr::mutate(Subject=gsub("^.*C7H2-(.*) treated.*$","\\1",title)) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="chALL") %>%
  dplyr::mutate(treatment_drug=ifelse(grepl("EtOH",title),"baseline","dex")) %>%
  dplyr::mutate(treatment_cell=ifelse(grepl("resistant",title),"resist","sens")) %>%
  dplyr::mutate(Treatment=paste0(treatment_drug,"_",treatment_cell)) %>%
  dplyr::mutate(Treatment_des=ifelse(grepl("EtOH",title),NA,"Dex 100nM 6h")) %>%
...
disease_con1 <- NULL
disease_con0 <- NULL
treatment_con1 <- c("dex_resist","dex_sens")
treatment_con0 <- c("baseline_resist","baseline_sens")
```
