---
title: 'Quality Control for Gene Expression Microarray Data -- GSE15823'
...
```{r eval=T, warning=F}
cols <- c("description","geo_accession","characteristics_ch1.1")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(GEO_ID=geo_accession) %>%
  dplyr::mutate(Subject=ifelse(grepl("asthma",description),ifelse(grepl("\\d",description),gsub("^.* (\\d+).*$","\\1",description),"00"),"control")) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="Lung") %>%
  dplyr::mutate(Disease=ifelse(grepl("allergic asthma",description),"allergic_asthma","healthy")) %>%
  dplyr::mutate(Treatment=ifelse(grepl("ICS",description),"ICS","baseline")) %>%
  dplyr::mutate(Gender=gsub("gender: ","",characteristics_ch1.1)) %>%
...
```{r eval=T}
disease_con1 <- c("allergic_asthma")
disease_con0 <- c("healthy")
treatment_con1 <- c("ICS")
treatment_con0 <- c("baseline")
```
