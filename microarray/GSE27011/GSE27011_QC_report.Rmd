---
title: 'Quality Control for Gene Expression Microarray Data -- GSE27011'
...
```{r eval=F, warning=F}
cols <- c("title","geo_accession","source_name_ch1","characteristics_ch1.1","characteristics_ch1.2")
pheno.new <- pheno.df %>%
  dplyr::select(cols) %>%
  dplyr::mutate(GEO_ID=geo_accession) %>%
  dplyr::mutate(Subject=title) %>%
  dplyr::mutate(Sample=paste(geo_accession,Subject,sep="_")) %>%
  dplyr::mutate(Tissue="WBC") %>%
  dplyr::mutate(Disease=ifelse(grepl("Mild asthma",source_name_ch1),"mild_asthma",ifelse(grepl("Severe asthma",source_name_ch1),"severe_asthma","healthy"))) %>%
  dplyr::mutate(Age=gsub("age: ","",characteristics_ch1.1)) %>%
  dplyr::mutate(Gender=gsub("Sex: ","",characteristics_ch1.2)) %>%
...
```{r eval=T}
disease_con1 <- c("mild_asthma","severe_asthma")
